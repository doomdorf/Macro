...
#if defined(NDEBUG)
# error "Macro cannot be compiled without assertions."
#endif
...
int nCoinbaseMaturity = 40; // macrocoin::: => 40
...
const string strMessageMagic = "Macro Signed Message:\n"; // macrocoin::: => Macro signed ...
...
{
    {
        ...
        CTxDB txdb("r");
        CTxIndex txindex;
      // macrocoin:::  if (tx.ReadFromDisk(txdb, COutPoint(hash, 0), txindex))
...
{
    int64_t nSubsidy = 3125000 * COIN; // macrocoin::: => 3125000 coins per block (till block 800 it gets close to 2mil totalcoins)
...
static const int64_t nTargetTimespan = 16 * 60;  // 16 mins macrocoin::: => same as black
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProtocolV2(nHeight) && nVersion < 7) // macrocoin::: 
        return DoS(100, error("AcceptBlock() : reject too old nVersion = %d", nVersion));

  //  else if (!IsProtocolV2(nHeight) && nVersion > 6) // macrocoin::: same
